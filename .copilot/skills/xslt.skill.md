...
* **Naming convention.** Use lower case words separated by hyphens (`kebab-case` or `dash-case`) for naming XSLT templates, functions, modes, and variables. The naming of elements output should follow the conventions of the output language being generated, for example, `camelCase` for XML elements used in the TEI standard.
