...
sub pgdp2sgml
{
	...
	$string =~ s/\[\^(ae|�)\]/\&aecirc;/g;				# ae with circumflex (FRANCK: using �)
	...
	if ($useExtensions == 1)  # Extensions used for FRANCK
	{
		...
		$string =~ s/\[\^�\]/\&aumlcirc;/g;					# a with dieresis and circumflex (FRANCK: using �)
		$string =~ s/\[\^�\]/\&oumlcirc;/g;					# o with dieresis and circumflex (FRANCK: using �)
		$string =~ s/\[\^�\]/\&uumlcirc;/g;					# u with dieresis and circumflex (FRANCK: using �)

		$string =~ s/\[\=�\]/\&amacracu;/g;					# a with macron and acute (FRANCK: inverted order!)
	}
