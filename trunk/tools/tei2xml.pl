...
sub processFile
{
	...
	system ("patc -p $toolsdir/ent2ucs.pat tmp.4 $basename.xml");
	...
	system ("patc -p $toolsdir/ent2ucs.pat tmp.4a $basename-words.html");
	system ("rm tmp.4a");
	# system ("perl ../../../tools/tei2txt.pl $basename.words > $basename-words.txt");
