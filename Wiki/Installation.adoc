...
For Linux, you probably already have Perl installed. For installing additional modules, I advise to use `cpanminus`, which can be installed with `sudo apt install cpanminus` and then invoked with `cpanm -S <package name>`.

=== Packages used

* Image::Size
* Lingua::BO::Wylie (Only for Tibetan support: download from https://www.lotsawahouse.org/Static/tools/ewts.html[www.lotsawahouse.org])
* HTML::Entities;
* Text::Levenshtein::XS
...
If you are missing a package, it can easily be installed using CPAN using cpanminus: `cpanm install &lt;package&gt;`.
...
SX is an SGML to XML translator, and NSGML is an SGML validator, part of the SP package. Since XSLT only supports XML, you will need both those two tools to be able to work with SGML. They can be downloaded from http://www.jclark.com/[James Clark website]. For windows, get ftp://ftp.jclark.com/pub/sp/win32/sp1_3_4.zip[sp 1.3.4]. On Linux, the easiest approach is to install `opensp` (`sudo apt install opensp`) and then create symbolic links from `onsgmls` and `osx` (see below)
...
As an alternative to SX and NSGML, you can use `osx` and `onsgml` from the OpenJade Project. These are slightly more up-to-date. You will either have to update the names in the main `tei2html.pl` script in that case, or create a symbolic link, using:

[source,bash]
----
sudo ln -s /usr/bin/onsgmls /usr/bin/nsgmls
sudo ln -s /usr/bin/osx /usr/bin/sx
----
...
For schematron, I use the schxslt implementation available from https://codeberg.org/SchXslt.

Just put the file `schxslt-cli.jar` in `tei2html/tools/lib`, and the main script will use it to generate a validation report of your TEI file (work-in-progress).

== Zopflipng and QRencode

_Optional, you will probably not need those tools, unless you produce texts for Project Gutenberg._

For generating a QR code (that contains an URL pointing at the book at Project Gutenberg), I use QRencode (Source at https://github.com/fukuchi/libqrencode[GitHub]).

On Linux, say `sudo apt install qrencode` to obtain it; on Windows `winget install -e --id PedroAlbanese.QREncode` will install it.

For compressing PNG images, I use `zopflipng`, which offers some of the best possible compression. This can be obtained
via https://github.com/google/zopfli[GitHub].

On Linux, just clone the git repository, and run `make`.
...
On Linux, you can add them to your `.bashrc` file, for example:

[source,bash]
----
PATH="/home/jeroen/Documents/Repositories/tei2html/tools${PATH:+:${PATH}}"; export PATH;
TEI2HTML_HOME="/home/jeroen/Documents/Repositories/tei2html/"; export TEI2HTML_HOME;
SAXON_HOME="/home/jeroen/Documents/Repositories/tei2html/bin"; export SAXON_HOME;
----
