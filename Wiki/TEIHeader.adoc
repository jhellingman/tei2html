...
`tei2html` uses the metadata in the TEI header to construct the metadata in the target formats, as well as the optional colophon. It is important to correctly specify the metadata in the TEI header.
...
The first element of the TEI header is the file description, `&lt;fileDesc&gt;`. This contains the most important metadata in three main elements, the `&lt;titleStmt&gt;`, `&lt;publicationStmt&gt;`, and `&lt;sourceDesc&gt;`. Our template looks like this.
...
[source,xml]
----
    <author key="Rijn, Rembrandt Harmenszoon van" ref="https://viaf.org/viaf/64013650">Rembrandt van Rijn (1606-1669)</author>
